...
describe('arena prize table', () => {
  it('pays the jackpot only for a championship', () => {
    expect(arenaPrize('champion', 1)).toBe(10000);
    expect(arenaPrize('final', 2)).toBe(3000);
    expect(arenaPrize('final', 3)).toBe(1500);
    expect(arenaPrize('final', 5)).toBe(500);
    expect(arenaPrize('q2out', 6)).toBe(200);
    expect(arenaPrize('q1out', 7)).toBe(200);
  });
});
...
describe('runTournament', () => {
  it('is deterministic for the same seed + pool', () => {
    const a = runTournament(snap(STRONG), 999, [], 60, '2026-7-16');
    const b = runTournament(snap(STRONG), 999, [], 60, '2026-7-16');
  ...
  it('stops at the round where the player is eliminated', () => {
    ...
    for (let s = 0; s < 40; s++) {
      const res = runTournament(snap(WEAK), s, [], 60, 'd');
  ...
  it('stored round order matches a fresh simulate2 (so playback lines up)', () => {
    const res = runTournament(snap(STRONG), 314, [], 60, 'd');
  ...
  it('a strong horse wins championships more often than a weak one', () => {
    ...
    let weakWins = 0;
    for (let s = 0; s < 60; s++) {
      if (runTournament(snap(STRONG), s, [], 60, 'd').outcome === 'champion') strongWins++;
      if (runTournament(snap(WEAK), s, [], 60, 'd').outcome === 'champion') weakWins++;
    }
